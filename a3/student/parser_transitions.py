...
def minibatch_parse(sentences, model, batch_size):
    ...
    while len(unfinished_parses) > 0:
        predicted_transitions = model.predict(unfinished_parses[:batch_size])
        for transition, parse in zip(predicted_transitions, unfinished_parses[:batch_size]):
            parse.parse_step(transition)

        unfinished_parses = [partial_parse for partial_parse in partial_parses if not (len(partial_parse.stack) == 1 and len(partial_parse.buffer) == 0)]
